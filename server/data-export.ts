...
export class DataExporter {
  static async exportSandwichCollections(options: ExportOptions = { format: 'csv' }) {
    try {
      ...
      const data = await query.orderBy(desc(sandwichCollections.collectionDate));

      if (options.format === 'csv') {
        return this.convertToCSV(data, [
          'id', 'hostName', 'collectionDate', 'sandwichCount', 'notes', 'createdAt'
        ]);
      }
  ...
  static async exportHosts(options: ExportOptions = { format: 'csv' }) {
    try {
      ...
      if (options.format === 'csv') {
        return this.convertToCSV(data, [
          'id', 'name', 'address', 'status', 'capacity', 'notes', 'createdAt'
        ]);
      }
    ...
    try {
      ...
      if (options.format === 'csv') {
        return this.convertToCSV(data, [
          ...
          'assigneeName', 'dueDate', 'progressPercentage', 'createdAt'
        ]);
      }
  ...
  static async exportAuditLogs(options: ExportOptions = { format: 'csv' }) {
    try {
      ...
      if (options.format === 'csv') {
        return this.convertToCSV(data, [
          'id', 'action', 'tableName', 'recordId', 'userId', 
          'ipAddress', 'timestamp'
        ]);
      }
  ...
  static async exportFullDataset(options: ExportOptions = { format: 'json' }) {
    try {
      const [
        ...
        db.select().from(sandwichCollections).orderBy(desc(sandwichCollections.collectionDate)),
        ...
        db.select().from(messages).orderBy(desc(messages.createdAt)).limit(1000)
      ]);
  ...
  private static convertToCSV(data: any[], fields: string[]): { data: string, format: 'csv' } {
    if (!data.length) {
